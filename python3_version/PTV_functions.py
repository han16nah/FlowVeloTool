...
def EstimateExterior(gcpCoo_file, imgCoo_GCP_file, interior_orient, estimate_exterior,
    ...
    try:
        ...
        if estimate_exterior:
            ...
            eor_mat = photogrF.getExteriorCameraGeometry(gcpImgPts_undist, gcpObjPts_table, interior_orient, unit_gcp,
...
def searchMask(waterlevel_pt, waterlevel_buffer, AoI_file, ptCloud, unit_gcp, interior_orient,
    ...
    # ...or calculate from water level information and 3D point cloud
        ...
        searchMask = detectF.defineFeatureSearchArea(pointsBelowWater, interior_orient, eor_mat, False, savePlotData,
                                                     directoryOutput, img_list[1])  # xy
...
def FeatureDetectionPTV(dir_imgs, img_list, frameCount, minimumThreshBrightness, neighborSearchRadius_FD, searchMask,
    ...
    featuresToTrack = detectF.featureDetection(dir_imgs, img_list[frameCount], searchMask, minimumThreshBrightness,
                                               neighborSearchRadius_FD,
                                               maximumNeighbors_FD, maxFtNbr_FD, sensitiveFD, savePlotData,
    ...
    featuresToTrack_id = np.hstack(
        (feature_ID.reshape(feature_ID.shape[0], 1), featuresToTrack[:, 1].reshape(featuresToTrack.shape[0], 1)))
    ...
    outputFileFD = open(
        os.path.join(directoryOutput, 'FD_every_' + str(FD_everyIthFrame) + '_' + img_list[frameCount][:-4]) + '.txt',
        'w')
...
def FeatureDetectionLSPIV(dir_imgs, img_list, frameCount, pointDistX, pointDistY, searchMask, FD_everyIthFrame,
    ...
    featuresToTrack = detectF.LSPIV_features(dir_imgs, img_list[frameCount], searchMask, pointDistX, pointDistY,
                                             savePlotData,
                                             directoryOutput)
    feature_ID = np.array(range(featuresToTrack.shape[0]))
...
def FeatureTracking(template_width, template_height, search_area_x_CC, search_area_y_CC, shiftSearchFromCenter_x,
                    shiftSearchFromCenter_y,
                    frameCount, FT_forNthNberFrames, TrackEveryNthFrame, dir_imgs, img_list, featuresToTrack,
                    interior_orient,
                    performLSM, lsmBuffer, threshLSM, subpixel, trackedFeaturesOutput_undist, save_gif, imagesForGif,
                    directoryOutput,
    ...
    trackedFeatures0_undist_px = photogrF.metric_to_pixel(trackedFeatures0_undist, interior_orient.resolution_x,
                                                          interior_orient.resolution_y,
        ...
        if lk:
            ...
            try:
                ...
                if initialEstimatesLK == True:
                    ...
                    x_initialGuess = x_initialGuess.reshape(x_initialGuess.shape[0], 1) + np.ones(
                        (featureEstimatesNextFrame.shape[0], 1)) * shiftSearchFromCenter_x
                    y_initialGuess = y_initialGuess.reshape(y_initialGuess.shape[0], 1) + np.ones(
                        (featureEstimatesNextFrame.shape[0], 1)) * shiftSearchFromCenter_y
                ...
                # perform tracking
                trackedFeaturesLK, status = trackF.performFeatureTrackingLK(templateImg, searchImg,
                                                                            featuresToTrack[:, 1:],
                                                                            initialEstimatesLK,
                                                                            featureEstimatesNextFrame,
                                                                            template_width, template_height,
                                                                            maxDistBackForward_px)
                ...
                trackedFeature_undist_px = photogrF.metric_to_pixel(trackedFeature_undist, interior_orient.resolution_x,
                                                                    interior_orient.resolution_y,
                                                                    interior_orient.sensor_size_x,
                                                                    interior_orient.sensor_size_y)

                frameName = np.asarray(
                    [img_list[img_nbr_tracking + TrackEveryNthFrame] for x in range(trackedFeaturesLK_px.shape[0])])
                trackedFeaturesOutput_undistArr = np.hstack((frameName, trackedFeaturesLK_px[:, 0]))
                trackedFeaturesOutput_undistArr = np.hstack(
                    (trackedFeaturesOutput_undistArr, trackedFeature_undist_px[:, 0]))
                trackedFeaturesOutput_undistArr = np.hstack(
                    (trackedFeaturesOutput_undistArr, trackedFeature_undist_px[:, 1]))
            ...
            for featureToTrack in featuresToTrack:
                ...
                try:
                    # perform tracking
                    trackedFeature_px = trackF.performFeatureTracking(template_size, search_area, featureToTrack[1:],
                                                                      templateImg, searchImg,
                                                                      shiftSearchArea, performLSM, lsmBuffer, threshLSM,
                                                                      subpixel, False)

                    # check backwards
                    trackedFeature_pxCheck = trackF.performFeatureTracking(template_size, search_area,
                                                                           trackedFeature_px, searchImg, templateImg,
                                                                           -1 * shiftSearchArea, performLSM, lsmBuffer,
                                                                           threshLSM, subpixel, False)
                    ...
                    # undistort tracked feature measurement
                    trackedFeature_undist = photogrF.undistort_img_coos(trackedFeature_px.reshape(1, 2),
                                                                        interior_orient)
                    trackedFeature_undist_px = photogrF.metric_to_pixel(trackedFeature_undist,
                                                                        interior_orient.resolution_x,
                                                                        interior_orient.resolution_y,
                                                                        interior_orient.sensor_size_x,
                                                                        interior_orient.sensor_size_y)
                    trackedFeaturesOutput_undist.append(
                        [img_list[img_nbr_tracking + TrackEveryNthFrame], int(featureToTrack[0]),
...
def FilterTracks(trackedFeaturesOutput_undist, img_name, directoryOutput,
    ...
    filteredFeatures.to_csv(directoryOutput + 'TracksFiltered_after_minmax_filter_angles.txt', sep='\t', index=False)
    ...
    print('nbr features after range angle filter: ' + str(nbr_features_rangeangle))

    # filter tracks outside main flow direction
    filteredFeatures, flowdir_angle = filterF.TrackFilterMainflowdirection(filteredFeatures, binNbrMainflowdirection,
                                                                           MainFlowAngleBuffer)
    ...
    return filteredFeatures, [nbr_features_raw, nbr_features_mindist, nbr_features_maxdist, nbr_features_mincount,
                              steady_angle, nbr_features_steady, range_angle, nbr_features_rangeangle, flowdir_angle,
                              nbr_features_mainflowdir]
...
def TracksPx_to_TracksMetric(filteredFeatures, interior_orient, eor_mat, unit_gcp,
    ...
    if lspiv:
        trackingDuration = np.ones((id_features.shape[0], 1), dtype=np.float) * TrackEveryNthFrame / np.float(
            frame_rate_cam)
    else:
    ...
    if lspiv:
        ...
        drawF.draw_tracks(filteredFeaturesPIV.groupby('id', as_index=False).mean(), image, directoryOutput,
                          'TracksReferenced_raw_PIV.jpg', 'velo', True)
        ...
        while featureCount < len(filteredFeaturesId) - 1:
            filteredFeaturesPIV.loc[filteredFeaturesPIV.id == filteredFeaturesId.loc[featureCount], 'veloMean'] = \
            filteredFeatureMean.loc[featureCount, 'velo']
            filteredFeaturesPIV.loc[filteredFeaturesPIV.id == filteredFeaturesId.loc[featureCount], 'veloStd'] = \
            filteredFeaturesStd.loc[featureCount, 'velo']
        ...
        if not cellsizeFilter == 0:
            ...
            drawF.draw_tracks(filteredFeaturesRaster, image, directoryOutput, 'filteredTracks_raster_PTV.jpg', 'velo',
                              True)
        ...
        drawF.draw_tracks(filteredFeaturesLocally, image, directoryOutput, 'filteredTracks_locally_PTV.jpg', 'velo',
                          True)
